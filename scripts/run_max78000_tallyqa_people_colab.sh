...
MODEL_NAME="${MODEL_NAME:-ai85tallyqambv3smallpeople}"
DATASET_NAME="${DATASET_NAME:-tallyqa_people_count_fold2_56}"
QAT_POLICY="${QAT_POLICY:-policies/qat_policy_tallyqa_people.yaml}"
SCHEDULE="${SCHEDULE:-policies/schedule-tallyqa-people.yaml}"
EPOCHS="${EPOCHS:-50}"
...
DRY_RUN="${DRY_RUN:-0}"
REPORT_DIR="${REPORT_DIR:-artifacts/reports/max78000/tallyqa_people_training}"

usage() {
  cat <<'EOF'
Usage:
  scripts/run_max78000_tallyqa_people_colab.sh [options]

Runs the current MAX78000 people-count training scaffold from edge_vlm, while
...
if [[ "${MATERIALIZE}" == "1" || "${MATERIALIZE}" == "true" ]]; then
  materialize_args=(
    uv run python scripts/materialize_max78000_people_dataset.py
...
if [[ "${STAGE}" == "1" || "${STAGE}" == "true" ]]; then
  stage_args=(
    uv run python scripts/stage_max78000_people_pipeline.py
...
if [[ "${MODEL_REPORT}" == "1" || "${MODEL_REPORT}" == "true" ]]; then
  model_report_args=(
    uv run python scripts/report_max78000_people_model.py
...
if [[ "${SETUP_AI8X_ENV}" == "1" || "${SETUP_AI8X_ENV}" == "true" ]]; then
  ...
  run_shell "cd '${ai8x_abs}' && uv pip install -r requirements-base.txt -r requirements-datasets.txt pycocotools==2.0.8"
  if [[ -d "${ai8x_abs}/distiller" ]]; then
    run_shell "cd '${ai8x_abs}' && uv pip install -e distiller --config-settings editable_mode=strict"
  else
