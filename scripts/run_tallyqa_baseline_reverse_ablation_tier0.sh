...
# 05: add local soft hard-label targets.
run_one "tallyqa-tier0-05-plus-local-soft-targets" \
  "data.require_teacher_cache=false" \
  "paths.teacher_cache=${TEACHER_CACHE}" \
  "data.train_sampling=prompt_class_tempered" \
  "data.prompt_class_sampling_temperature=0.5" \
...
# 06: add composite-teacher KL on top of local soft targets.
run_one "tallyqa-tier0-06-plus-composite-teacher-kl" \
